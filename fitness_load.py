...
def load_fitness(per_classnum, seed, thred=0.1):
    """
    基于history模板修改的Fitness数据集加载器
    字段: category, text, label, node_id, neighbour
    """
    args = set_params()

    # 1. 加载原始数据
    df_orig = load_and_preprocess_data('./dataset/Fitness/Fitness.csv', per_classnum)

    # 2. 加载改写数据（需确保文件存在）
    try:
        df_rewritten = load_and_preprocess_data(f"./dataset/Fitness/Fitness_ratio{args.ratio}.csv",
                                                per_classnum)
    ...
    test_val_size = ceil(len(remaining) * 1.0)  #
